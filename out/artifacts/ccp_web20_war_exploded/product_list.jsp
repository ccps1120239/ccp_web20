...
<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
	<ul class="pagination" style="text-align: center; margin-top: 10px;">
		...
		<c:if test="${pageBean.pageCurrent != 1}">
			<li>
				<a href="${pageContext.request.contextPath}/productListServlet?pageCurrent=${pageBean.pageCurrent -1}"
				   aria-label="Previous">
					<span aria-hidden="true">&laquo;</span>
				</a>
			</li>
		...
		<c:forEach begin="1" end="${pageBean.totalPage}" var="page">
			<%--判断当前页--%>
			<c:if test="${pageBean.pageCurrent == page}">
				<li class="active"><a href="javascript:void(0);">${page}</a></li>
			</c:if>
			<c:if test="${pageBean.pageCurrent != page}">
				<li><a href="${pageContext.request.contextPath}/productListServlet?pageCurrent=${page}">${page}</a></li>
			</c:if>
