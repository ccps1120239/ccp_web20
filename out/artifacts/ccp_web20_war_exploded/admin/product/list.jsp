<%@ page language="java" pageEncoding="UTF-8" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<HTML>
<HEAD>
	<meta http-equiv="Content-Language" content="zh-cn">
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	<link href="${pageContext.request.contextPath}/css/Style1.css"
	      rel="stylesheet" type="text/css"/>
	<script language="javascript" src="${pageContext.request.contextPath}/js/public.js"></script>
	<script language="javascript" src="${pageContext.request.contextPath}/js/jquery-1.11.3.min.js"></script>
	<script type="text/javascript">
      function addProduct() {
          window.location.href = "${pageContext.request.contextPath}/adminAddProductUIServlet";
      }

      /**
       * 点击删除
       * @param pid 商品的pid
       * @returns {boolean}
       */
      function deleteProduct(pid, pname) {
          if (confirm("你确定要删除【" + pname + "】吗？")) {
              location.href = "${pageContext.request.contextPath}/adminDelProductServlet?pid=" + pid;
              alert("【" + pname + "】删除成功");
          }
      }

      /**
       * 编辑操作
       */
      function updateProductInfo(pid) {
          location.href = "${pageContext.request.contextPath}/adminUpdateProductUIServlet?pid=" + pid;
          <%--location.href =  "${pageContext.request.contextPath}/admin/product/edit.jsp";--%>
      }

      /**
       * 批量删除操作
       */
      function deleteProducts() {
          var obj = document.getElementsByName("hobby");
          var check_val = [];
          for (var k in obj) {
              if (obj[k].checked)
                  check_val.push(obj[k].value);
          }
          // alert(check_val);
          if (confirm("你确定要删除吗？")) {
              alert("删除成功");
              location.href = "${pageContext.request.contextPath}/adminDeleteProductsServlet?pid=" + check_val;
          }
      }

      /**
       * 全选
       */
      function checkAllProducts() {
          var element = document.getElementById("allCheck");
          if (element.checked == true) {
              var hobbies = document.getElementsByName("hobby");
              for (var i = 0; i < hobbies.length; i++) {
                  hobbies[i].checked = true;
              }
          } else {
              var hobbies = document.getElementsByName("hobby");
              for (var i = 0; i < hobbies.length; i++) {
                  hobbies[i].checked = false;
              }
          }

      }

      /**
       * 搜索的数据回显
       */
      $(function () {
		      $("#hot option[value='${condition.isHot}']").prop("selected",true);
		      $("#cid option[value='${condition.cid}']").prop("selected",true);
      })
	</script>
</HEAD>
<body>
<br>
<%--表单--%>
<form id="Form1" name="Form1" action="${pageContext.request.contextPath}/adminSearchProductServlet" method="post">
	<%-- 查询 --%>
	商品名称：<input type="text" name="pname" value="${condition.pname}">&nbsp;&nbsp;
	是否热门：<select name="isHot" id="hot">
						<option value="">全部</option>
						<option value="1">是</option>
						<option value="0">否</option>
					</select>&nbsp;&nbsp;
	商品类别：<select name="cid" id="cid">
						<option value="">全部</option>
						<c:forEach items="${categoryList}" var="category">
							<option value="${category.cid}">${category.cname}</option>
						</c:forEach>
					</select>&nbsp;&nbsp;
		<input type="submit" value="搜索">
	<%--表格--%>
	<table cellSpacing="1" cellPadding="0" width="100%" align="center"
	       bgColor="#f5fafe" border="0">
		<TBODY>
		<tr>
			<td class="ta_01" align="center" bgColor="#afd1f3"><strong>商品列表</strong>
			</TD>
		</tr>
		<tr>
			<td class="ta_01" align="right">
				<button type="button" id="add" name="add" value="添加"
				        class="button_add" onclick="addProduct()">
					添加
				</button>
			</td>
		</tr>
		<%--<tr>--%>
		<%--<td class="ta_01" align="right">--%>
		<%--<button type="button" id="allCheck" name="allCheck" value="全选"--%>
		<%--class="button_all" onclick="checkAllProducts()">--%>
		<%--全选--%>
		<%--</button>--%>
		<%--</td>--%>
		<%--</tr>--%>
		<tr>
			<td class="ta_01" align="right">
				<button type="button" id="del" name="del" value="删除"
				        class="button_del" onclick="deleteProducts()">
					删除
				</button>
			</td>
		</tr>
		<tr>
			<td class="ta_01" align="center" bgColor="#f5fafe">
				<table cellspacing="0" cellpadding="1" rules="all"
				       bordercolor="gray" border="1" id="DataGrid1"
				       style="BORDER-RIGHT: gray 1px solid; BORDER-TOP: gray 1px solid; BORDER-LEFT: gray 1px solid; WIDTH: 100%; WORD-BREAK: break-all; BORDER-BOTTOM: gray 1px solid; BORDER-COLLAPSE: collapse; BACKGROUND-COLOR: #f5fafe; WORD-WRAP: break-word">
					<tr
							style="FONT-WEIGHT: bold; FONT-SIZE: 12pt; HEIGHT: 25px; BACKGROUND-COLOR: #afd1f3">

						<td align="center" width="18%">
							<input type="checkbox" name="hobby" id="allCheck" onclick="checkAllProducts()">
							序号
						</td>
						<td align="center" width="17%">商品图片</td>
						<td align="center" width="17%">商品名称</td>
						<td align="center" width="17%">商品价格</td>
						<td align="center" width="17%">是否热门</td>
						<td width="7%" align="center">编辑</td>
						<td width="7%" align="center">删除</td>
					</tr>

					<c:forEach items="${productList}" var="product" varStatus="vs">
						<tr onmouseover="this.style.backgroundColor = 'white'" onmouseout="this.style.backgroundColor = '#F5FAFE';">
							<td style="CURSOR: hand; HEIGHT: 22px" align="center" width="18%">
								<input type="checkbox" name="hobby" value="${product.pid}">
									${vs.count}
							</td>
							<td style="CURSOR: hand; HEIGHT: 22px" align="center" width="17%">
								<img width="40" height="45" src="${pageContext.request.contextPath}/${product.pimage}">
							</td>
							<td style="CURSOR: hand; HEIGHT: 22px" align="center" width="17%">${product.pname}</td>
							<td style="CURSOR: hand; HEIGHT: 22px" align="center" width="17%">${product.shop_price}</td>
							<td style="CURSOR: hand; HEIGHT: 22px" align="center" width="17%">${product.is_hot == 1 ?"是":"否"}</td>
							<td align="center" style="HEIGHT: 22px">
								<a href="#" onclick="updateProductInfo('${product.pid}')">
									<img src="${pageContext.request.contextPath}/images/i_edit.gif" border="0" style="CURSOR: hand">
								</a>
							</td>
							<td align="center" style="HEIGHT: 22px">
									<%--
									href="javascript:void(0);
									a标签默认有绑定事件。
									这段代码大意指：点击执行绑定事件后，a标签默认的绑定事件不重新做任何事。就是啥都别干！
									--%>
								<a href="javascript:void(0)" onclick="deleteProduct('${product.pid}','${product.pname}')">
									<img src="${pageContext.request.contextPath}/images/i_del.gif" width="16" height="16" border="0"
									     style="CURSOR: hand">
								</a>
							</td>
						</tr>
					</c:forEach>

				</table>
			</td>
		</tr>
		</TBODY>
	</table>
</form>
</body>
</HTML>

